...
# Chapter9. Assembling the Application

- 모든 의존성은 도메인 코드를 향해야 함. 그래야 도메인이 외부에 영향 받지 않음.
- 만약 유스케이스가 영속성 어댑터가 필요해서 직접 new 키워드를 통해 생성했다고 가정. 이 경우 앞서 말한 의존성의 방향이 잘못됨.
- 영속성 어댑터의 인터페이스를 추출하고, 의존성을 역전시킴으로써 앞서 말한 의존성의 방향을 지킴.
- 이는 테스트를 쉽게 하는 부가적인 효과도 있음.

그렇다면 객체를 생성하는 주입하는 책임은 누가 지는가?

- 저자는 아키텍처에 중립적이고 모든 클래스를 인스턴스화 하기 위해 모두를 알고 있는, 그런 Confuguration Component 가 있어야 한다고 말함.
- 이 컴포넌트는 객체를 생성하고 필요한 곳에 주입해주는 역할을 함. 또한 각종 설정 파일에 접근할 수 있음. (커맨드라인 파라미터, 컨피그 파일 등)
- 스프링의 ApplicationContext 가 하는 일이기도 함.

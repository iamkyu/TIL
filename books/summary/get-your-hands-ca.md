...
- 인상적이었던 것은 Repository 자체를 서비스 계층에 노출하지 않으면서 필요한 인터페이스만 노출하도록 한 것
- 트랜잭션 처리는 가장 쉬운 방법은 스프링에서 제공하는 Transactional 어노테이션을 사용하는 것. 서비스계층이 특정 프레임워크 종속되지 않도록 하고 싶지만 AOP 방식으로 직접 처리할 수도.

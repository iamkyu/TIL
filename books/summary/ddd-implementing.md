...
도메인에서 발생한 사건을 포착하기 위해 도메인 이벤트를 사용. 다음과 같은 말을 할 때 명시적인 이벤트를 모델링을 할 가능성이 큼.
- “…할 때”
- “그런 일이 일어나면…”
- “…하면 저에게 알려주세요.” 와 “…하면 저에게 통보해주세요.”
- “…한 일의 발생”

![이벤트의 흐름](https://user-images.githubusercontent.com/13076271/47251278-5822ce80-d46d-11e8-8b22-9dd58059a05e.png)

### 이벤트 모델링
- 해당 이벤트가 속한 바운디드 컨텍스트의 유비쿼터스 언어에 따라 이벤트와 속성을 명명.
- 현재 일어나고 있는 일이 아닌 과거에 일어난 사실이라는 것을 이름에 나타내는 것이 좋음.
- 이벤트가 언제 일어났는지 나타내는 타임스탬프 필요.
- 해당 이벤트를 다시 발생시키기 위해 필요한 다른 추가 속성이 무엇인지 고려.
- 이벤트는 해당 원인을 반영한 속성을 운반하려는 목적을 가짐.

### 도메인 모델에서 이벤트 발행하기
- 도메인 모델은 어떤 종류의 미들웨어 메시징 인프라로 노출되지 않아야 함.
- 메시징 관련 컴포넌트는 오직 인프라 내에 위치.
- 또한 도메인 모델이 이런 인프라를 간접적으로 사용할 땐 절대 명시적 결합*coupling*을 형성하지 않아야 함.
- 가장 간단하고 효과적인 방법은 경량 옵저버의 생성.

### 반복 메세지의 또는 순서가 지켜지지 않은 메세지의 처리
1. 구독자 모델의 오퍼레이션이 멱등하도록 설계
2. 구독자가 영속성 메커니즘을 통해 메세지를 저장하여 처리 전 확인
3. 메세지에 ID를 부여해 순서를 확인
4. 메세지의 타임스탬프를 통해 특정 시점 이후 이벤트만 적용



## 9장 모듈

모듈은 서로 간에 높은 응집도를 갖고 있는 도메인 객체를 담는 이름이 붙여진 컨테이너의 역할을 수행, 각기 다른 모듈에 있는 클래스 사이의 낮은 결합도를 유지하는 것이 목표.

### 모듈 설계의 간단한 규칙
- 모델링 개념에 맞춰 모듈 설계
- 유비쿼터스 언어에 맞춰 모듈 명명
	- 새로운 모듈을 생성할 때, 기존 모듈의 이름까지도 대담하게 바꿀 수 있을 정도로 공격적으로 접근
- 모델에서 사용하는 일반적인 컴포넌트 타입이나 패턴에 따라 기계적으로 모델을 생성하지 말 것
	- 예를 들어 모든 에그리겟을 하나의 모듈, 모든 서비스를 하나의 모듈 으로 분리하는 등
- 느슨하게 결합된 모듈 설계
- 결합이 필요하다면 짝이 되는 모듈 사이에서 비순환적 의존성이 형성되도록 노력
	- 하지만 자식 모듈과 부모 모듈 사이에 규칙은 느슨하게 적용할 수도 있음
- 모듈을 모델의 정적인 개념에 따라 만들지 말고 모듈이 담고 있는 객체에 맞춤

### 기본 모듈 명명 규칙
- 고유한 최상위 수준 이름을 사용함으로써 다른 프로젝트 모듈과 네임스페이스 충돌 방지 `io.iamkyu`
- 상업적 제품 이름(브랜드)을 모듈 이름에 사용하지 말 것
	- 브랜드 이름은 바뀔 수 있으며, 때로 제품 이름과 근간을 이루는 바운디드 컨텍스트 사이에 직접적 연관이 거의 없을 때가 있음

...
- [테스트 주도 개발로 배우는 객체 지향 설계와 실천](#테스트-주도-개발로-배우는-객체-지향-설계와-실천)
    ...
    - [22장 복잡한 테스트 데이터 만들기](#22장-복잡한-테스트-데이터-만들기)
        - [테스트 데이터 빌더](#테스트-데이터-빌더)
        - [팩터리 메서드를 이용한 도메인 모델 강조](#팩터리-메서드를-이용한-도메인-모델-강조)
...
## 22장 복잡한 테스트 데이터 만들기
‘코드는 읽으려고 있는 것’  이라는 원칙 아래 테스트 데이터를 만드는 과정에 데이터 빌더를 적용하여 다음 이득을 취하는 과정을 소개 함.

- 좀 더 읽기 쉽고 기능의 의도를 서술하는 선언적 테스트 작성.
- 이렇게 작성 된 수준 높은 테스트를 업무 분석가처럼 기술과 관련 없는 이해 당사자와 직접적으로 의사소통하는데 활용할 수 있음. (*이 문장은 어떤 의도를 가지는지 잘 이해가지 않음)
...
public void chargesCustomerForTotalCostOfAllOrderedItems() {
	...
	new Customer("홍길동",
                    new Address("송파구 방이동 1", 
                            "서울",
                            new PostCode("138", "050"))));
    	order.addLine(*new OrderLine("볼캡", 1);
    	order.addLine(*new OrderLine("벙거지 모자", 1));
}

// Test Logic
```

- 생성자와 불변 값 객체를 사용하는데 엄격한 구조라면 테스트에서 객체를 생성하는 일이 수고로움. 테스트에서는 객체를 생성하고 싶을 때마다 생성자 인자를 모두 직접 제공해야 하기 때문.
- 이 모든 객체를 생성하는 코드는 테스트를 읽기 어렵게 만듬.
- 객체 구조가 변경됐을 때 이런 생성 방식을 사용하는 여러 테스트가 깨짐. 즉 불안정한 테스트.
...
- 테스트 데이터 생성에 빌더 패턴을 적용하여 빌더의 장점을 취함. 해당 패턴으로 취할 수 있는 장점은 [John Grib 님의 Wiki 페이지](https://johngrib.github.io/wiki/builder-pattern/) 에 잘 설명되어 있음.
- 현재 테스트에 중요하지 않은 데이터는 빌더내에서 기본값으로 초기화하고 일부 속성은 선택적으로 값을 전달할 수 있음.
- 위 예제는 ‘빌더’ 자체를 인자로 넘김. 이로 인해 `build()` 메서드를 호출할 필요가 없고 뭔가를 만드는 메커니즘이 아닌 현재 생성 중인 대상이 부각되어 결과가 더 읽기 쉬워짐.
...
- 위 예제는 빌더를 생성하는 부분을 팩터리 메서드로 감싸서 잡음을 더욱 줄임.
- 메서드 중복정의를 활용해 `with()`  메서드 하나로 줄이고 파라미터 타입으로 구분.
- 따라서 도메인 타입 도입이 촉진되어 좀 더 표현력 있어짐.
- 이 과정으로도 테스트 코드가 읽기 어렵다면 테스트 대상 객체가 너무 많은 의존 관계를 가지지 않는지, 비대한 생성자를 가진지 않는지 등 을 점검해보면 좋을 듯.
